...
def _update_latest_prototypes_pointer(gcs_storage, timestamp: str):
    ...
    pointer_blob = gcs_storage.bucket.blob(f"indexes/prototypes_latest.json")
...
def restore_prototypes_from_gcs(timestamp: Optional[str] = None) -> Optional[Dict]:
    ...
    try:
        ...
        if timestamp is None:
            # Get latest version
            try:
                pointer_blob = gcs.bucket.blob(f"indexes/prototypes_latest.json")
                pointer_data = json.loads(pointer_blob.download_as_text())
                timestamp = pointer_data["latest_timestamp"]
            except Exception:
                return None
...
def list_prototype_versions() -> List[str]:
    ...
    try:
        ...
        blobs = gcs.client.list_blobs(gcs.bucket_name, prefix=f"indexes/prototypes_")
        timestamps = []
...
def cleanup_old_backups_all() -> Dict[str, any]:
    ...
    results = {"positive": {"cleaned": 0, "kept": 0}, "negative": {"cleaned": 0, "kept": 0}, "prototypes": {"cleaned": 0, "kept": 0}, "errors": []}
